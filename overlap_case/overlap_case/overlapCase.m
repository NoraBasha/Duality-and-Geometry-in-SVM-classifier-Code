...
% SVM- HW#3-Q4- Overlapping case
%====================================================================================
%Loading training and testing data
S = load('C:\Fall 2019\Optimization\HW\overlap_case\overlap_case\train_overlap');
A=S.A;
B=S.B;
S2 = load('C:\Fall 2019\Optimization\HW\overlap_case\overlap_case\test_overlap');
test=S2.X_test;
True_labels=S2.true_labels;
%Finding the closest two points u,v of the two convex hull of class A and B
% Contraints are chnged to reduce the convex hull of each class to overcome
% the overlapping
cvx_begin
    variable u(2)
    variable v(2)
...
figure
hold on
plot3(y1,Classifier,y2,'DisplayName','Classifier Plan view with training data, overlapping is obvious')
scatter(A(1,:),A(2,:),'DisplayName','Class A')
scatter(B(1,:),B(2,:), 'DisplayName','Class B')
hold off
